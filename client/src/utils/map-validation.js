...
export const mapSanityResult = (barcodesDict) => {
  var sanity = [];
  var neighbourStructureConsistency = checkNeighbourStructureConsistency(
    barcodesDict
  );
  var ifAisleToStorageAndVisavis = checkIfAisleToStorageAndVisavis(
    barcodesDict
  );
  ...
  if (neighbourStructureConsistency.length != 0) {
    sanity.push({
      check_neighbour_structure_consistency: neighbourStructureConsistency,
    });
  }
  if (ifAisleToStorageAndVisavis.length != 0) {
    sanity.push({
      check_if_aisle_to_storage_and_visavis: ifAisleToStorageAndVisavis,
    });
  }
  ...
  if (alignmentOfCoordinates.length != 0) {
    sanity.push({
      number_of_wrongly_aligned_barcodes: alignmentOfCoordinates.length,
      wrongly_aligned_barcodes: alignmentOfCoordinates,
    });
...
export const checkNeighbourStructureConsistency = (barcodesDict) => {
  ...
  for (var coordinateKey in barcodesDict) {
    ...
    [[e0, b0, r0], [e1, b1, r1], [e2, b2, r2], [e3, b3, r3]] = barcodesDict[
      coordinateKey
    ].neighbours;
    var invalidMovement =
      (e0 == 0 && b0 + r0 > 0) ||
      ...
      (b2 == 0 && r2 > 0) ||
      (b3 == 0 && r3 > 0);
    var nonExistentNeighbour = getNonExistentNeighbour(
      barcodesDict[coordinateKey].neighbours,
      coordinateKey,
      barcodesDict
    );
...
export const getNonExistentNeighbour = (
  neighbours,
  coordinateKey,
  barcodesDict
) => {
  ...
  for (var neighbour_stucture in neighbours) {
    ...
    if (inDirNeigh[0] == 0) {
      ...
      nonExistentNeighbour =
        nonExistentNeighbour ||
        getNeighbourInDirection(
          coordinateKey,
          neighbour_stucture,
          barcodesDict
        ) == null;
...
export const getNeighbourInDirection = (
  coordinateKey,
  direction,
  barcodesDict
) => {
  if (isValidCoordinateKey(coordinateKey)) {
    ...
    if (barcodesDict[coordinateKey].adjacency == undefined) {
      var transPortInfoInDir =
        barcodesDict[coordinateKey].neighbours[direction];
...
export const checkIfAisleToStorageAndVisavis = (barcodesDict) => {
  ...
  for (var coordinateKey in barcodesDict) {
    if (barcodesDict[coordinateKey].store_status == 1) {
      var neighbourCoordinates = getAllExistingNeighbours(
        coordinateKey,
        barcodesDict
      );
      for (var neighbour in neighbourCoordinates) {
        if (
          barcodesDict[neighbourCoordinates[neighbour]] != undefined &&
          barcodesDict[neighbourCoordinates[neighbour]].store_status == 1
        ) {
          // finalInvalidDataAcc.push([]);
        } else {
          var result = getToAndFroRackMovementMismatch(
            coordinateKey,
            neighbourCoordinates[neighbour],
            barcodesDict
          );
          if (result != undefined) {
...
export const getToAndFroRackMovementMismatch = (
  coordinateKey1,
  coordinateKey2,
  barcodesDict
) => {
  ...
  var Coor1ToCoor2Dir = getDirection(
    coordinateKey2,
    coordinateKey1,
    barcodesDict
  );
  ...
  if (rack1 != rack2) {
    return [
      { coordinateKey1, Coor1ToCoor2Dir },
      { coordinateKey2, Coor2ToCoor1Dir },
    ];
  }
};
...
export const getDirection = (
  ...
  if (barcodesDict[sourceCoordinateKey].adjacency == undefined) {
    return getDirectionUsingCoordinate(
      destinationCoordinateKey,
      sourceCoordinateKey
    );
  } else {
    if (
      barcodesDict[sourceCoordinateKey].adjacency.indexOf(
        destinationCoordinateKey
      ) == -1
    ) {
      return getDirectionUsingCoordinate(
      ...
      return barcodesDict[sourceCoordinateKey].adjacency.indexOf(
        destinationCoordinateKey
      );
    }
  }
...
export const getDirectionUsingCoordinate = (
  destinationCoordinateKey,
  sourceCoordinateKey
) => {
...
export const checkDeadGridPositions = (barcodesDict) => {
  ...
  for (var coordinateKey in barcodesDict) {
    var neighbourListWithLiftUp = getAccessibleNeighboursInAllDirectionsStatic(
      coordinateKey,
      "up",
      barcodesDict
    );
    var neighbourListWithLiftDown = getAccessibleNeighboursInAllDirectionsStatic(
      coordinateKey,
      "down",
      barcodesDict
    );
    ...
    for (var neighbourCordDir in validNeighbours) {
      ...
      try {
        var neighbourToCoordDir = getDirection(
          coordinateKey,
          neighbourCord,
          barcodesDict
        );
        ...
        if (isDeadGridDown == true && d == 1 && isDeadGrid == true && b == 1) {
          wrongCoordinates.push([
            { coordinateKey, oppNeighbourToCoordDir },
            { neighbourCord, neighbourToCoordDir },
          ]);
        } else if (
          isDeadGridDown == false &&
          isDeadGridUp == true &&
          u == 1 &&
          isDeadGrid == true &&
          b == 1
...
export const getAccessibleNeighboursInAllDirectionsStatic = (
  coordinateKey,
  liftState,
  barcodesDict
) => {
  ...
  for (var dir in direction) {
    ...
    if (
      nthDirectionNeighbour[0] + nthDirectionNeighbour[1] == 2 &&
      liftState == "down"
    ) {
      if (barcodesDict[coordinateKey].adjacency == undefined) {
      ...
      nthDirectionNeighbour[0] +
        nthDirectionNeighbour[1] +
        nthDirectionNeighbour[2] ==
      3
    ) {
      if (barcodesDict[coordinateKey].adjacency == undefined) {
        result.push(getExistingNeighbourInDirection(coordinateKey, dir));
...
export const validateAlignmentOfCoordinates = (barcodesDict) => {
  ...
  for (var coordinateKey in barcodesDict) {
    ...
    var coordinate = coordinateKeyToTupleOfIntegers(
      barcodesDict[coordinateKey].coordinate
    );
    var refWorldCoordinate =
      barcodesDict[coordinateKey].world_coordinate_reference_neighbour;
    ...
    [0,1,2,3].forEach((val) => {
      ...
      if (neighbourInDir != null) {
        var worldCoordinateInDir =
          barcodesDict[neighbourInDir].world_coordinate;
        var worldCoordinateParsed = JSON.parse(worldCoordinateInDir);
        var neighbourCoordinate = coordinateKeyToTupleOfIntegers(
          barcodesDict[neighbourInDir].coordinate
        );
        if (
          (val % 2 == 0 && worldCoordinateParsed[0] != x) ||
          (val % 2 == 1 && worldCoordinateParsed[1] != y)
        ) {
